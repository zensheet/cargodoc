...
//     'sales_order' sekarang ikut auto-enable untuk self-signup,
...
(async function initSalesOrder() {
  const { allowed, session, guest } = await requireFeatureOrGuest('sales_order');
...
async function loadSoForEdit(id) {
  ...
  if (heading) heading.innerHTML =
    `<a href="/app.html" style="text-decoration:none;color:inherit;"><img src="assets/logo-cargodoc.webp" alt="CargoDoc" style="height:18px;vertical-align:middle;"></a> / Edit Sales Order ${so.so_number}`;
...
async function saveOnly() {
  ...
  if (IS_GUEST) {
    guestAuthGate(async () => {
      ...
      showActivationModal({ justSaved: true, onClose: () => location.href = '/sales-order-list.html' });
    });
    return;
  }
...
async function saveAndDownload() {
  ...
  if (IS_GUEST) {
    ...
    const previewSo = { ...data.sales_order, so_number: data.sales_order.so_number || 'PREVIEW' };
    generateSalesOrderPDF({ ...data, sales_order: previewSo, branding: null, watermark: true });

    // 2) Modal signup/login ringan. Draft (`data`) baru benar-benar
    //    disimpan ke DB SETELAH auth sukses.
    guestAuthGate(async () => {
      const saved = await persistSalesOrder(data, 'final');
      const branding = await getBranding();
      // sql/26-hybrid-trial.sql: baru signup -> trial 7 hari & 5 dokumen LANGSUNG aktif,
      ...
      if (watermark) {
        ...
        alert('✅ Account created & sales order saved. PDF downloaded.');
        location.href = '/sales-order-list.html';
      }
    });
    return;
